...
export function setup() {
  for (const u of [U1, U2]) {
    const r = http.post(`${BASE_URL}/api/auth/register`, JSON.stringify(u), {
...
function login(u) {
  const r = http.post(`${BASE_URL}/api/auth/login`, JSON.stringify(u), {
    headers: { 'Content-Type': 'application/json' },
  });
...
export default function () {
  ...
  const tr = http.post(
    ...
    { headers: { 'Content-Type': 'application/json', 'X-Session': s1 } }
  );
  ...
  const tr2 = http.post(
    ...
    { headers: { 'Content-Type': 'application/json', 'X-Session': s2 } }
  );
